...
(function() {
  ...
  function refreshAllPills() {
    ...
    document.querySelectorAll('.bst-zip-pill .bst-zip-display').forEach(function(el) { el.textContent = label; var p = el.closest('.bst-zip-pill'); if (p) p.setAttribute('aria-label', 'Change location, currently ' + label); });
  ...
  function wirePill(pill) {
    ...
    pill.setAttribute('aria-label', cityMode ? 'Change location, currently ' + cityMode.name + ' — shop all cars near you' : 'Change location, currently ' + displayText);
